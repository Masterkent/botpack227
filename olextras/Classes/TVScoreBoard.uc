...
function ShowScores( canvas Canvas )
{
  ...
  if (scoreholder != None)
  {
    ...
    if (scoreholder.ENali > 0)
      DrawBodyCount("'Evil' Nali", scoreholder.ENali, Canvas, row++);
    ...
    if (scoreholder.FriendlyDamage>0){
      Canvas.DrawColor = Red;
      DrawBodyCount("Damage Inflicted on allies", scoreholder.FriendlyDamage, Canvas, row++);
      Canvas.DrawColor = White;
    }
    ...
    if (scoreholder.KilledByFollowers>0)
      DrawBodyCount("Kills by Allies", scoreholder.KilledByFollowers, Canvas, row++);
    if (scoreholder.KilledFollowers>0){
      Canvas.DrawColor = Red;
      DrawBodyCount("Allies Dead", scoreholder.KilledFollowers, Canvas, row++);
      Canvas.DrawColor = White;
    }
    row++;
