...
function Paint(Canvas C, float X, float Y)      //Draw UT HUD crosshair preview
{
  ...
  if (H==none){
    ClipText(C, ShowHUD.WinLeft, CrosshairSlider.WinTop + 20, "Operation: Na Pali no longer active!  Changes will not be saved!", false);
    return;
  }
  CrossHair = H.CrosshairTextures[H.tvCrosshair];
  if(CrossHair == None)
    CrossHair = H.LoadCrosshair(H.tvCrosshair);
...
function CrosshairChanged()  //ol crosshair stuff.....
{
  if (TVHUD(GetPlayerOwner().Myhud)!=none)
    TVHUD(GetPlayerOwner().myHUD).tvCrosshair = int(CrosshairSlider.Value);
}
