...
function scoreit(pawn WhatDied){
  if (WhatDied.IsA('Brute'))
    ...
    Gasbags++;
  else if (WhatDied.IsA('Krall'))
    Krall++;
  else if (/*WhatDied.IsA('Mercenary')||*/WhatDied.Isa('followingMercenary'))
    Mercs++;
    ...
    Titans++;
  else if (WhatDied.IsA('Skaarjtrooper')||WhatDied.IsA('rebelskaarjtrooper'))
    Skaarjt++;
  else if (WhatDied.IsA('Skaarjwarrior')||WhatDied.IsA('rebelskaarj'))
    Skaarjw++;
  else if (WhatDied.IsA('nalirabbit')||WhatDied.IsA('cow'))
    Animals++;
  else if (WhatDied.IsA('Brute'))
    Brutes++;
  else if (WhatDied.IsA('nali')||(WhatDied.IsA('nalitrooper')&&Follower(WhatDied).IsFriend()))
    Nali++;
