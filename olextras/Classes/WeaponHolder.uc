...
// C) Have a nice spot for new ONP followers or enemies.
// ============================================================

class WeaponHolder expands Follower
  abstract;

var () class<weapon> WeaponType; //class of weapon
var (Follower) bool Evil; //if true, is enemy. (implemented here, as all subclasses are "new" enemies...)
var weapon MyWeapon; //my weapon ;p

replication{  //needed?  Test later!
...
simulated function PreBeginPlay(){
  super.PreBeginPlay();
  if (Evil)  //so never likes player.
     bCoward=true;
  bHasRangedAttack = false;
  bMovingRangedAttack = false;
}

function bool DoTravel(tvplayer Traveler, int ArrayNum){  //weapon support
  if (Super.DoTravel(Traveler,ArrayNum)){
      Traveler.Friendlies[ArrayNum]*=10;
      Traveler.friendlies[ArrayNum]+=(weapon.inventorygroup%10)*10;
      return true;
  }
}

simulated function bool IsFriend(){
  if (Evil)
    return false;
  return Super.IsFriend();
}

function PreGreeting(){ //don't fire!
  Super.PreGreeting();
  bfire=0;
  baltfire=0;
  if ( Weapon != None )
...
function TossWeapon()
{
  if ( Weapon == None )
    return;
  Weapon.AimError = Weapon.default.AimError;
...
auto state Startup
{
  ...
  function SetHome()
  {
    ...
    else if (WeaponType==class'enforcer') //quick hack
...
defaultproperties
{
     WeaponType=Class'olextras.NoammoDpistol'
