...
// ScriptedHuman : A Scripted Pawn human. Can be follower, or enemy (evil=true)
// ===============================================================
...
//insane method of copying over skins and texture. bah and pah
function bool DoTravel(tvplayer Traveler, int ArrayNum){
  if (Super.DoTravel(Traveler,ArrayNum)){
     Traveler.friendlynames[ArrayNum]=Myname$chr(17)$string(mesh)$
      chr(17)$string(multiskins[0])$chr(17)$string(multiskins[1])$chr(17)$string(multiskins[2])
       $chr(17)$string(multiskins[3])$chr(17)$string(FollowerIcon)$chr(17)$string(Voice);
      return true;
  }
}
...
//decodes the following thing set above:
function ParseSkinInfo(string Info){
  local int i, j;
  i=instr(Info,chr(17));
  MenuName=left(Info,i);
  CutInfo(Info,i);
  mesh=Mesh(DynamicLoadObject(left(Info,i),class'mesh'));
  for (j=0;j<4;j++){
    CutInfo(Info,i);
    Multiskins[j]=Texture(DynamicLoadObject(left(info,i),class'Texture',true));
  }
  CutInfo(Info,i);
  FollowerIcon=Texture(DynamicLoadObject(left(info,i),class'Texture'));
  CutInfo(Info,i);
  Voice=class<challengevoicepack>(DynamicLoadObject(info,class'class'));
}
//AI/speech stuff
...
function PlayGreetAnim(){
  ...
  if (bShouldWait)
    ...
    PlayVoice(2,3,true); //got back
  else
    PlayVoice(2,15,true); //got covered
}
...
function damageAttitudeTo(pawn Other)       //overload for FF stuff.
{
  if ( other.bisplayer && IsFriend() && Level.TimeSeconds - LastPainSound >= 0.25 && frand()<0.5*float(enemy==none)+0.2){
    PlayVoice(1,-1,true);
