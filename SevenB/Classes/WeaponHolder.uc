// ============================================================
// WeaponHolder : This class is designed for two things:
// A) Have a base class of weapon holders.
// B) Prevent melee attacks
// ============================================================

class WeaponHolder expands ScriptedPawn
  abstract;

var () class<weapon> WeaponType[5]; //class of weapon (allow 5 of them)
...
{
	if (bImmuneToHeadShot && (HitLocation.Z - Location.Z > 0.62 * CollisionHeight))
		Damage=0;
	super.TakeDamage(Damage,instigatedBy,hitlocation,momentum,damageType);
}

// toss out a weapon currently held
function TossHeldWeapon(weapon weap)
{
  local vector X,Y,Z;
  if ( weap == None )
    return;
  GetAxes(Rotation,X,Y,Z);
  weap.DropFrom(Location + 0.8 * CollisionRadius * X + - 0.5 * CollisionRadius * Y);
}
...
function Died(pawn Killer, name damageType, vector HitLocation)
{
  ...
  if (bNoDropWeapon){
   if (weapon!=none)
 	...
 	else{ //drop all weapons
		for (inv=inventory;inv!=none;inv=inv.inventory)
			if(inv.IsA('weapon') && (inv.Class!=Level.Game.BaseMutator.MutatedDefaultWeapon()) && weapon(inv).bCanThrow )
 			{
   			speed = VSize(Velocity);
   			weap = Weapon(inv);
		    if (speed != 0)
     			weap.Velocity = Normal(Velocity/speed + 0.5 * VRand()) * (speed + 280);
   			else {
    		  weap.Velocity.X = 0;
		      weap.Velocity.Y = 0;
		      weap.Velocity.Z = 0;
...
//1337 dodging?  (new-ish code now detects if would jump off a plank.. because that is ungood)
...
function FireWeapon()     //multi-weapon support
{
...
defaultproperties
{
     WeaponType(0)=Class'olweapons.OLDpistol'
