...
//sound imports:
/*
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsJump.WAV" NAME="XiJump"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsDie1.WAV" NAME="XiDie1"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsDie2.WAV" NAME="XiDie2"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsDie3.WAV" NAME="XiDie3"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsDieDecapitated.WAV" NAME="XiDecap"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsDrowned.WAV" NAME="XiDrown"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsForwardFlip.WAV" NAME="XiDodge"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsGasp1.WAV" NAME="XiGasp1"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsGasp2.WAV" NAME="XiGasp2"
//#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsHumming.WAV" NAME="XiHumm"
//#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjured1.WAV" NAME="XiHit1"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjured2.WAV" NAME="XiHit2"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjured3.WAV" NAME="XiHit3"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjured4.WAV" NAME="XiHit4"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjured5.WAV" NAME="XiHit5"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsInjuredInWater.WAV" NAME="XiUWHit"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsLand.WAV" NAME="XiLand1"
#exec AUDIO IMPORT FILE="Sounds\XidiaSpenceSoundsLand2.WAV" NAME="XiLand2"
*/
//ship model:
/*
#exec mesh import mesh=shuttle anivfile=Models\shuttle_a.3d datafile=Models\shuttle_d.3d x=0 y=0 z=0
#exec mesh origin mesh=shuttle x=0 y=0 z=0 Yaw=192 ROLL=0 PITCH=0
#exec mesh sequence mesh=shuttle seq=All startframe=0 numframes=1

#exec meshmap new meshmap=shuttle mesh=shuttle
//#exec meshmap scale meshmap=shuttle x=0.50195 y=0.50195 z=1.00391
#exec meshmap scale meshmap=shuttle x=0.13 y=0.13 z=0.26
	*/
var bool checkwall;
...
var SevenLevelInfo LInfo; //info for some options. set on server, but client must get it manually.....
...
exec function GetWeapon(class<Weapon> NewWeaponClass )
{
  ...
  switch(NewWeaponClass){
    case class'PulseGun':
      NewWeaponClass = class'SevenPulsegun';
      break;
    case class'ShockRifle':
      NewWeaponClass = class'SevenShockRifle';
      break;
    case class'Minigun2':
      NewWeaponClass = class'SevenChainGun';
      break;
    case class'Enforcer': //or not :p
      NewWeaponClass = class'SevenMachineMag';
      break;
    case class'SniperRifle':
      NewWeaponClass = class'SevenSniperRifle';
      break;
    case class'UT_FlakCannon':
      NewWeaponClass = class'SBFlechetteCannon';
      break;
    case class'UT_Eightball':
      NewWeaponClass = class'TVeightball';
      break;
    case class'ripper':
      NewWeaponClass = class'SBBloodRipper';
      break;
  }
...
exec function KillAll(class<actor> aClass)
{
  ...
  if (ScoreHolder!=none&&!ClassIsChildOf(aClass,class'Weapon')) //UsAar33: quick hack for EMP field..
...
/*-exec function lastBlood(){ //more testing
	if (tvshadow(shadow)!=none){
		log ("last blood texture spawned by player's shadow was"@tvshadow(shadow).lastBloodTex);
		clientmessage ("last blood texture spawned by player's shadow was"@tvshadow(shadow).lastBloodTex);
	}
}*/
...
function DoJump( optional float F )
{
  ...
  if ( CarriedDecoration != None || bForceWalk)
    return;
...
function ClientDrunk(float DrunkTime){ //set how long client is drunk
...
function UTF_PlayHit(float Damage, vector HitLocation, name damageType, vector Momentum)
{
  ...
  switch (damageType){
  	Case 'jolted':      //shock
  		if (frand()<0.7) //only drunk a 3/10 of time
  			return;
		case 'Electrocuted':
			clientDrunk(4);
  		break;
  	case 'FlakDeath': //flak alt (add flak normal?)
  		if (frand()<0.5) //50% of time
  			ClientDrunk(6);
  		break;
  	case 'Decapitated': //hit in head
			ClientDrunk(9); //ouch!
			break;
  }
}
 ...
 //  aTurn *=75/airspeed;
   ...
   // clientmessage(int(airspeed)@"KM/H",'pickup',false); //fun way of sending data :D
    ...
    if ( Role < ROLE_Authority ) // then save this move and replicate it
      SB_ReplicateMove(DeltaTime, Acceleration, DODGE_None, rot(0,0,0));
    else
...
//    return;
   ...
   if (role==role_authority&&weapon!=none&&(bfire>0||(baltfire>0&&!weapon.IsA('sevensniperrifle')))&&headregion.zone.IsA('GunFireSensorZone'))
