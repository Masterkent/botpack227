    ...
    for (i=0;i<20;i++){
      if (i>7&&Linfo.DefaultInventory[i]==none)
        ...
        if (inv.class==class'SevenMachineMag'&&Linfo.bAkimboMags)
          SevenMachineMag(inv).hastwoMag=true;
        if (inv.IsA('Weapon'))
...
function RegisterSevenLevelInfo(actor newinfo){
  ...
  log ("Successfully bound level information",'SevenB');
}
//new playerpawn:
...
function BeginPlay(){ //use VA!
  ...
  if (level.netmode==nm_dedicatedserver){
    m=class<mutator>(DynamicLoadObject("SBVa.VaServer",class'class'));
    if (m!=none)
      Spawn(m);
  }
}
...
function ScoreKill(pawn Killer, pawn Other)    //Use singleplayer scoring system
{
  ...
  if ((Killer==none||Killer==Other)&&(Other.IsInState('TriggerAlarm')||Other.Enemy==none||(!Other.Enemy.bIsPlayer)))
