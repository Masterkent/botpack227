...
// ShockWave.
//=============================================================================
class M79ShockWave extends Effects;
		...
		if ( Level.NetMode == NM_Client )
		{
			foreach VisibleCollidingActors( class 'Actor', Victims, ShockSize*29, Location )
				if ( Victims.Role == ROLE_Authority )
				{
					...
					{
						MoScale = FMax(0, 275 - 1.1 * Dist);
						Victims.Velocity = Victims.Velocity + dir * (MoScale + 20);
						Victims.TakeDamage(MoScale,Instigator,Victims.Location - 0.5 * (Victims.CollisionHeight + Victims.CollisionRadius) * dir,(1 * dir),'grenadeDeath');
					}
				}
			return;
		}
	...
	foreach VisibleCollidingActors( class 'Actor', Victims, ShockSize*29, Location )
	{
		...
		if (dist> OldShockDistance || (dir dot Victims.Velocity < 0))
		{
			...
			if ( Victims.bIsPawn )
				...
				Victims.Velocity = Victims.Velocity + dir * (MoScale + 20);
			Victims.TakeDamage(MoScale,Instigator,Victims.Location - 0.5 * (Victims.CollisionHeight + Victims.CollisionRadius) * dir,(1000 * dir),'grenadeDeath');
		}
	}
...
simulated function SpawnEffects()
{
	 ...
	 PlaySound(Sound'addweap.m79.m79exploze', SLOT_Interface, 16.0);
	 PlaySound(Sound'addweap.m79.m79exploze', SLOT_None, 16.0);
	 PlaySound(Sound'addweap.m79.m79exploze', SLOT_Misc, 16.0);
	 PlaySound(Sound'addweap.m79.m79exploze', SLOT_Talk, 16.0);
...
defaultproperties
{
     ...
     Mesh=LodMesh'addweap.ShockWaveX'
     AmbientGlow=255
     bUnlit=True
}
