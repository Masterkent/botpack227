	...
	if ( AmmoType.UseAmmo(1))
	{
		...
		if ( bRapidFire ||(FiringSpeed > 0) )
			Pawn(Owner).PlayRecoil(FiringSpeed);
                ...
                if ( bInstantHit  &&  Pawn(owner).velocity.x < -100 || bInstantHit  &&  Pawn(owner).velocity.x > 200 || bInstantHit  && Pawn(owner).velocity.y < -100 || bInstantHit  && Pawn(owner).velocity.y > 100 )
			TraceFire(1.2);
...
function TraceFire( float Accuracy )
{
	...
	Other = Pawn(Owner).TraceShot(HitLocation,HitNormal,EndTrace,StartTrace);
        ...
        Pawn(Owner).ViewRotation.Pitch += (shotc * 50);
   ...
   if ( (Other != self) && (Other != Owner) && (Other != None) )
	{
		...
		if ( Other.bIsPawn && (HitLocation.Z - Other.Location.Z > 0.62 * Other.CollisionHeight)
			...
			Other.TakeDamage(50, Pawn(Owner), HitLocation, 35000 * X, AltDamageType);
		else
			Other.TakeDamage(30,  Pawn(Owner), HitLocation, 30000.0*X, MyDamageType);
...
simulated function Zoom()
{
	...
	{
		case 55:
			PlayerPawn(Owner).DesiredFOV = 100;
			break;


		case 100:

			PlayerPawn(Owner).DesiredFOV = 55;
			break;
	}
}
...
defaultproperties
{
     ...
     FireSound=Sound'AX.Sounds.mp5'
     AltFireSound=Sound'AX.Sounds.Reload3'
     SelectSound=Sound'Botpack.enforcer.Cocking'
     Misc1Sound=Sound'AX.Sounds.Reload4'
     ...
     PickupMessage="You got the M-4 Assault Rifle."
     ItemName="M-4 Assault Rifle"
     PlayerViewOffset=(X=11.600000,Y=-3.900000,Z=-5.700000)
     PlayerViewMesh=LodMesh'AX.Asm4'
     PlayerViewScale=0.300000
     BobDamping=0.975000
     PickupViewMesh=LodMesh'AX.asm4pickup'
     PickupViewScale=0.750000
     ThirdPersonMesh=LodMesh'AX.asm43rd'
     ThirdPersonScale=0.680000
     StatusIcon=Texture'AX.Icons.usem4'
     ...
     Icon=Texture'AX.Icons.usem4'
     Physics=PHYS_Falling
     Mesh=LodMesh'AX.asm43rd'
     bNoSmooth=False
