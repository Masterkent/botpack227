class MyNaliDefiler extends NaliDefiler;

var() float SkaarjSpawnRadius;
var() float MaxSkaarjWarriors;
...
{
	Loc.Z = Location.Z;
	Spawn(class'KKSkaarjWarrior',none,'',Loc);
	Spawn(class'UTTeleEffect',none,'',Loc);
}
...
Begin:
	mni=0;
SpawnSW:
	if( mni < MaxSkaarjWarriors || MaxSkaarjWarriors == 0 )
	{
                SpawnSkaarjW( Location+(VRand()*SkaarjSpawnRadius) );
		Sleep(2);
		GoTo('SpawnSW');
	}
	Sleep(3);
...
defaultproperties
{
     SkaarjSpawnRadius=512.000000
     MaxSkaarjWarriors=6.000000
}
