...
function Created()
{
	...
	Sens = string(NCHUD(getPlayerOwner().myHUD).spellmTime);
	i = InStr(Sens, ".");
	spellHide.SetValue(Left(Sens, i+3));
	ControlOffset += 25;
	...
	Sens = string(NCHUD(getPlayerOwner().myHUD).weaponmTime);
	i = InStr(Sens, ".");
	wepHide.SetValue(Left(Sens, i+3));
	ControlOffset += 25;
	...
	Sens = string(NCHUD(getPlayerOwner().myHUD).inventorymTime);
	i = InStr(Sens, ".");
	invHide.SetValue(Left(Sens, i+3));
	ControlOffset += 25;
	...
	ConvDelayCombo.SetSelectedIndex(NCHUD(getPlayerOwner().myHUD).convDelayType);
	ControlOffset += 25;
...
function Paint(Canvas C, float X, float Y)
{
	...
	if (GetPlayerOwner().myHUD.Crosshair==0)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair1');
	else if (GetPlayerOwner().myHUD.Crosshair==1)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair2');
	else if (GetPlayerOwner().myHUD.Crosshair==2)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair3');
	else if (GetPlayerOwner().myHUD.Crosshair==3)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair4');
	else if (GetPlayerOwner().myHUD.Crosshair==4)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair5');
	else if (GetPlayerOwner().myHUD.Crosshair==5)
		DrawClippedTexture(C, CrosshairX, 140, Texture'Crosshair7');
}
