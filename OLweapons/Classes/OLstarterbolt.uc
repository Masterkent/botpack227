// ============================================================
// Olweapons.olstarterbolt: used for skaarj trooper and nalitrooper (operation: na pali)
...
class OLstarterbolt expands starterbolt;
...
simulated function Tick(float DeltaTime)
{
  ...
  if ( Instigator != None )
  {
    ...
    else
    {
      ...
      else if (instigator.isa('scriptedpawn')){  //skaarj or nali
        mypawn=scriptedpawn(instigator);
       if ( Instigator.Target == None )
          Instigator.Target = Instigator.Enemy;
        if ( Instigator.Target == Instigator.Enemy&&instigator.target!=none )
        {
          dAdjust = DeltaTime * (4 + instigator.Skill) * 0.12;
          if ( OldError > NewError )
            OldError = FMax(OldError - dAdjust, NewError);
          ...
          if ( StartError > 0 )
            StartError -= DeltaTime;
          else if ( mypawn.skill<2 && (Level.TimeSeconds - mypawn.LastPainTime < 0.2) )
            StartError = mypawn.LastPainTime;
          else
            StartError = 0;
          AimSpot = 1.25 * Instigator.Target.Velocity + 0.75 * Instigator.Velocity;
          if ( Abs(AimSpot.Z) < 120 )
    ...
    if (instigator.isa('nali')){
        drawoffset.z-=9;
        drawoffset.y+=5.5;
        drawoffset.x+=14;
    }
    else if (instigator.isa('skaarj')){
        drawoffset.y+=59;
        drawoffset.z-=9;
    }
