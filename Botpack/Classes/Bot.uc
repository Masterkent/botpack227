...
function string KillMessage(name damageType, pawn Other)
{
	...
	return class'UTC_GameInfo'.static.UTSF_PlayerKillMessage(Level.Game, damageType, OtherPRI) $ PlayerReplicationInfo.PlayerName;
}
...
function SetPeripheralVision()
{
	...
	if (bSniping && AmbushSpot != none)
		SightRadius = float(AmbushSpot.GetPropertyText("SightRadius"));
	else
...
function ChangedWeapon()
{
	...
	if ( Weapon != None )
	{
		...
		/// Was: Weapon.SetHand(0);
		Weapon.SetHand(B227_Handedness);
...
function rotator AdjustAim(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
	...
	if ( Target.bIsPawn && (!bNovice || bDefendMelee) 
		&& (Weapon != None) 
		&& (Weapon.bRecommendSplashDamage || (UTC_Weapon(Weapon) != none && UTC_Weapon(Weapon).bRecommendAltSplashDamage && (bAltFire != 0))) 
...
{
	if ( !B227_HasAliveEnemy() || VSize(Enemy.Location - LastSeenPos) > 800 )
		return false;
...
function bool CanImpactJump()
{
	...
	return bHasImpactHammer && Health > 54 * DamageScaling + 6 && DamageScaling <= 1; // [B227 fix for UDamage]
}
...
function bool FindAmbushSpot()
{
	...
	if ( Ambushspot != None )
	{
		...
		if ( VSize(Ambushspot.Location - Location) < 2 * CollisionRadius )
		{
			...
			bSniping = ((Orders == 'Defend') && bool(AmbushSpot.GetPropertyText("bSniping")));
			CampTime = 10.0;
			SightRadius = float(AmbushSpot.GetPropertyText("SightRadius"));
...
state Roaming
{
	...
	function HitWall(vector HitNormal, actor Wall)
	{
		...
		Focus = Destination;
		if ( !bWallAdjust && B227_PickWallAdjust() )
		{
			if ( Physics == PHYS_Falling )
				SetFall();
			else
...
state Wandering
{
	...
	function HitWall(vector HitNormal, actor Wall)
	{
		...
		Focus = Destination;
		if ( B227_PickWallAdjust() && (FRand() < 0.7) )
		{
			if ( Physics == PHYS_Falling )
				SetFall();
			else
...
RunAway:
	...
	bAdvancedTactics = !bNovice && Level.TimeSeconds - LastSeenTime < 1.0 &&
						Skill > 2.5 * FRand() - 1 &&
						(NavigationPoint(MoveTarget) == none || !bool(MoveTarget.GetPropertyText("bNeverUseStrafing")));
SpecialNavig:
	if (SpecialPause > 0.0)
	{
		if ( LineOfSightTo(Enemy) )
...
{
	ignores HearNoise;
	//Was: ignores SeePlayer, HearNoise;
...
*/
	...
	function PickRegDestination(bool bNoCharge)
	{
		...
		else 
		{
			...
			{
				FireWeapon();
				if ( (bJumpy && (FRand() < 0.75)) || class'UTC_Weapon'.static.UTSF_SplashJump(Weapon) )
				{
					// try jump move
					SetPhysics(PHYS_Falling);
					Acceleration = vect(0,0,0);
	...
	function PickDestination()
	{
		...
		else
		{
			...
			if ( !FastTrace(LastSeenPos, ViewSpot) )
			{
				...
				if ( B227_PickWallAdjust() || FindViewSpot() )
				{
					if ( Physics == PHYS_Falling )
						SetFall();
					else
...
ignores EnemyNotVisible; 
	...
	function bool ClearShot()
	{
		...
		if (!B227_HasAliveEnemy() ||
			!FastTrace(LastSeenPos + vect(0,0,0.9) * Enemy.CollisionHeight, Location))
		{
			bFire = 0;
			bAltFire = 0;
			return false;
...
Begin:
	...
	if (ImpactHammer(Weapon) == none)
		GotoState(NextState, NextLabel);
	Sleep(0.5);
	if ( Physics != PHYS_Falling && ImpactHammer(Weapon) != none )
	{
		MakeNoise(1.0);
...
ignores Bump, Hitwall, WarnTarget;
	...
	function SeePlayer(Actor SeenPlayer)
	{
		if (!B227_bDisableSeePlayer)
		{
			Global.SeePlayer(SeenPlayer);
			///disable('SeePlayer');
			B227_bDisableSeePlayer = true;
			disable('HearNoise');
		}
		if (SeenPlayer == Enemy || LineOfSightTo(Enemy))
			LastSeenTime = Level.TimeSeconds;
...
state RangedAttack
{
	...
	// Was: ignores SeePlayer, HearNoise, Bump;
