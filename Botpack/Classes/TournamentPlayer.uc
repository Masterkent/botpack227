...
///var bool bNeedActivate; // [U227] Moved to TournamentWeapon
var bool b3DSound;
...
///var Weapon ClientPending; // [U227] Replaced with Pawn.PendingWeapon
///var Weapon OldClientWeapon; // [U227] Removed
...
exec function AddBotNamed(string BotName)
{
	...
	class'UTC_GameInfo'.static.UTSF_ForceAddBot(Level.Game);
}
...
function PlayWinMessage(bool bWinner)
{
	...
	B227_ClientPlayVoice(self, Announcement, true, true);
}
...
///function SendClientFire(weapon W, int N)           // [U227] Removed
///function SendClientAltFire(weapon W, int N)        // [U227] Removed
...
///function SendFire(Weapon W)                         // [U227] Removed
///function SendAltFire(Weapon W)                      // [U227] Removed
///function UpdateRealWeapon(Weapon W)                 // [U227] Removed
///function RealWeapon(weapon Real, int N)             // [U227] Removed
...
exec function Loaded()
{
	...
	B227_GiveUTArmor(class'ThighPads', !class'B227_Config'.default.bFixLoaded);
}
...
state FeigningDeath
{
	...
	function BeginState()
	{
		if (Role == ROLE_Authority && CTFFlag(PlayerReplicationInfo.HasFlag) != none)
			CTFFlag(PlayerReplicationInfo.HasFlag).Drop(vect(0, 0, 0));
		super.BeginState();
	}
}
...
{
	local TournamentPickup Inv;
	local int Armor2Charge;
	local int ThighPadsCharge;
	local int InvCharge;

	if (bIndependent)
	{
		Inv = Spawn(ArmorClass);
		if (Inv != none)
		{
			Inv.bHeldItem = true;
			Inv.RespawnTime = 0.0;
	...
	if (ArmorClass == class'Armor2')
		Armor2Charge = ArmorClass.default.Charge;
	if (ArmorClass == class'ThighPads')
		ThighPadsCharge = ArmorClass.default.Charge;
	InvCharge = class'TournamentPickup'.static.B227_HandleUTArmors(self, 0, Armor2Charge, ThighPadsCharge);

	if (InvCharge == 0)
		return;

	Inv = TournamentPickup(FindInventoryType(ArmorClass));
	if (Inv == none)
	{
		Inv = Spawn(ArmorClass);
		if (Inv == none)
