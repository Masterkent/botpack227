...
function ScoreFlag(Pawn Scorer, CTFFlag theFlag)
{
	...
	if ( Scorer.PlayerReplicationInfo.Team == theFlag.Team )
	{
		...
		for ( TeamMate=Level.PawnList; TeamMate!=None; TeamMate=TeamMate.NextPawn )
		{
			if (PlayerPawn(TeamMate) != none)
				class'UTC_PlayerPawn'.static.UTSF_ClientPlaySound(PlayerPawn(TeamMate), ReturnSound);
	...
	if (GoalTeamScore > 0 && Teams[Scorer.PlayerReplicationInfo.Team].Score >= GoalTeamScore)
