	...
	if ( bDelayedResponse )
	{
			...
			DelayedSender != none)
		{
			if ( PlayerPawn(Owner).GameReplicationInfo.bTeamGame
				...
				MessageType = 'Say';
			class'UTC_Pawn'.static.UTSF_TeamMessage(PlayerPawn(Owner), DelayedSender, DelayedResponse, MessageType, false);
		}
	}
...
function B227_PlayVoice(sound Sound)
{
	Owner.SetPropertyText("bIsVoicePlayer", "true");
	if ( (PlayerPawn(Owner).ViewTarget != None) && !PlayerPawn(Owner).ViewTarget.IsA('Carcass') )
	{
		PlayerPawn(Owner).ViewTarget.PlaySound(Sound, SLOT_Interface, 16.0);
		PlayerPawn(Owner).ViewTarget.PlaySound(Sound, SLOT_Misc, 16.0);
	}
	else
	{
		PlayerPawn(Owner).PlaySound(Sound, SLOT_Interface, 16.0);
