...
function RemoveFort(FortStandard F, Pawn instigator)
{
	...
	if (F.bSayDestroyed &&
		(Bot(instigator) != none || (TournamentPlayer(instigator) != none && TournamentPlayer(instigator).bAutoTaunt)))
	{
		class'UTC_Pawn'.static.UTSF_SendTeamMessage(instigator, none, 'OTHER', 16, 15);
	}
	else
