...
FFSlider = UWindowHSliderControl(CreateControl(class'UWindowHSliderControl', CenterPos, ControlOffset, CenterWidth, 1));
  ...
  FFSlider.SetText(class'UMenu.UMenuTeamGameRulesCWindow'.default.FFText$" ["$FFS*10$"%]:");
  FFSlider.SetHelpText(class'UMenu.UMenuTeamGameRulesCWindow'.default.FFHelp);
...
{
  local int ControlWidth, ControlLeft, ControlRight;
  local int CenterWidth, CenterPos, EditWidth;
...
function FFChanged()
{
  ...
  FFSlider.SetText(class'UMenu.UMenuTeamGameRulesCWindow'.default.FFText$" ["$int(FFSlider.GetValue()*10)$"%]:");
}
