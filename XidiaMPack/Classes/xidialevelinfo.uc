...
class XidiaLevelInfo expands Info;
...
var (Inventory) bool bAkimboMags; //whether automag should be forced into akimbo mode (if added)
...
function PreBeginPlay(){
  ...
  for (i=0;i<8;i++){
    if (InventoryToDestroy[i]==none)
      break;
    if (InventoryToDestroy[i]==class'UT_Jumpboots')
      InventoryToDestroy[i]=class'XidiaJumpBoots';
  }
  for (i=0;i<20;i++){
    if (DefaultInventory[i]==none)
      break;            //use case statements?
    else if (DefaultInventory[i]==class'ShockRifle'||DefaultInventory[i]==class'osShockRifle')
      DefaultInventory[i]=class'xidiaShockRifle';
    else if (DefaultInventory[i]==class'SniperRifle')
      DefaultInventory[i]=class'xidiaSniperRifle';
    ...
    else if (DefaultInventory[i]==class'UT_Jumpboots')
      DefaultInventory[i]=class'XidiaJumpBoots';
    else if (DefaultInventory[i]==class'automag'||DefaultInventory[i]==class'olautomag')
      DefaultInventory[i]=class'xidiaautomag';
...
defaultproperties
{
     ...
     DefaultInventory(1)=Class'XidiaMPack.TvFlashLight'
     bAkimboMags=True
