// ===============================================================
// XidiaMPack.TvSP: for new player and other stuff (called TVSP to save recoding time ;p)
// ===============================================================
...
event InitGame( string Options, out string Error )
{
  ...
  InOpt = ParseOption( Options, "XidiaMode" );
  if( InOpt != "" )
    XidiaMode = int(InOpt);
  super(UnrealGameInfo).InitGame(Options,Error);
}
//new playerpawn:
...
if (Linfo==none){ //check for info presence
  Log("No XidiaLevelInfo placed in level!  Using default options.",'Xidia');
  spawn(class'XidiaLevelInfo'); //muty will add to ginfo
}
CheckPlayerStarts();
...
function ScoreKill(pawn Killer, pawn Other)    //does not affect PRI stuff.
{
  ...
  if (Other==none||Other.bIsPlayer||((Killer==none||Killer==Other)&&(Other.Enemy==none||(!Other.Enemy.bIsPlayer))))
...
defaultproperties
{
     ...
     ScoreBoardType=Class'XidiaMPack.TVScoreBoard'
     HUDType=Class'XidiaMPack.TVHUD'
     GameName="Xidia ][: The Escape"
