...
//    DeltaRot=rotator(turnvec)-AimRotation;
  ...
  if (target != None) {
    turnvec=target.location+0.62 *vect(0,0,1)* target.CollisionHeight-BaseLoc;
    if (vsize(turnvec)>BeamSize*(MaxPos+1)||(pawn(target)!=none&&Pawn(target).health<=0)){
      Destroy(); //give up..
      return;
    }
...
defaultproperties
{
     SpriteAnim(0)=Texture'XidiaMPack.Skins.sbbolt0'
     SpriteAnim(1)=Texture'XidiaMPack.Skins.sbbolt1'
     SpriteAnim(2)=Texture'XidiaMPack.Skins.sbbolt2'
     SpriteAnim(3)=Texture'XidiaMPack.Skins.sbbolt3'
     SpriteAnim(4)=Texture'XidiaMPack.Skins.sbbolt4'
