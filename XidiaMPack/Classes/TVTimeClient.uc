...
function SetTime (string Time, bool esca){
  local int i, pos;
  pos=-1;
  if (esca)
    offs=class'XidiaPack'.default.ExpStart;
  for (i=0;i<7;i++){
    CutInfo(Time,pos);
    if (pos==-1)
      return;
...
function Paint(Canvas C, float X, float Y)
{
  ...
  for (i=0;i<7;i++){
    if (MapTimes[i]=="")
      return;
    WriteText(C, string(i+1)$". "$class'XidiaPack'.default.MapTitles[i+offs], Y);
